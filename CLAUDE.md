...
- Editorial rules: **no** numbered section markers, **no** uppercase tracked eyebrow above sections, **no** identical card grids, **no** hero stats-grid, **no scroll cue** (removed 2026-07-30 — "you know what scroll is", and its delayed fade was what tripped the axe contrast check), **no marquee/auto-scrolling ribbons** (the hero credibility strip is static since 2026-07-30; `Marquee.tsx` keeps the name but stands still), **no em dashes in visible copy** — all three bundles were swept clean 2026-07-30; keep it that way (commas, periods, parentheses; date ranges use a plain hyphen). The hero opens straight on the name (no eyebrow); the "currently building" availability line lives in the contact footer.
