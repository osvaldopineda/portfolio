...
export default function Hero() {
  return (
    <section id="top" className="grid-bg relative flex min-h-screen items-center overflow-hidden">
      {/* Ambient accent glow */}
      <div className="pointer-events-none absolute -right-40 top-10 h-[420px] w-[420px] rounded-full bg-accent/10 blur-[120px]" />
      ...
      <div className="mx-auto w-full max-w-6xl px-6 pt-28 pb-16">
        ...
        <motion.p {...fade(0.2)} className="serif-display mt-8 max-w-3xl text-3xl italic leading-snug text-bone md:text-4xl">
          {PROFILE.tagline}
        </motion.p>
        ...
        <motion.div {...fade(0.36)} className="mt-9 flex flex-wrap items-center gap-3">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-ink transition-transform active:scale-95"
          >
            <Download className="h-4 w-4" /> Download CV
          </a>
          <a href={`mailto:${PROFILE.email}`} className="flex items-center gap-2 rounded-full border border-line px-5 py-2.5 text-sm font-medium text-bone transition-colors hover:border-bone/40">
            <Mail className="h-4 w-4" /> Email
          </a>
          <a href={PROFILE.github} target="_blank" rel="noreferrer" aria-label="GitHub" className="flex h-11 w-11 items-center justify-center rounded-full border border-line text-bone transition-colors hover:border-bone/40">
            <Github className="h-4 w-4" />
          </a>
          <a href={PROFILE.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" className="flex h-11 w-11 items-center justify-center rounded-full border border-line text-bone transition-colors hover:border-bone/40">
